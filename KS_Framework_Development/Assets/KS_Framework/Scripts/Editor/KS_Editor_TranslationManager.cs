...
namespace KS_Core.Editor
{
    ...
    public class KS_Editor_TranslationManager : EditorWindow
    {
        ...
        private KeyValuePair<string, TranslationString>[] loadedLines = null;
        private string loadedString = null;

        [MenuItem("KS: Framework/Translation Manager", false, 21)]
        static void Init()
        {
            ...
            for (int i = 0; i < translations.languages.Count; i++)
            {
                if (translations.languages[i].strings.Count > 0)
                {
                    for (int j = 0; j < translations.languages[i].strings.Count; j++)
                    {
                        ...
                        {
                            lines.Add(new KeyValuePair<string, TranslationString>(translations.languages[i].language, translations.languages[i].strings[j]));
                        }
                    }
                }
            }
