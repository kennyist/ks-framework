...
public class KS_Mapping : KS_Behaviour {

    // Instancing

    private static KS_Mapping instance;
    public static KS_Mapping Instance
    {
        get
        {
    ...
    {
        target += Vector3.left * (KS_Input.GetAxis("mapMoveX") * (mapMoveSpeed * ScalePercent));
        target += Vector3.forward * (KS_Input.GetAxis("mapMoveY") * (mapMoveSpeed * ScalePercent));
    ...
    private void RayCastMouse()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3000f, uiMask))
        {
            ...
            {
                if (mapHoverObject == null && hit.collider.gameObject.GetComponent<KS_Map_Marker>())
                {
                    mapHoverObject = hit.collider.gameObject;
                    mapHoverObject.GetComponent<KS_Map_Marker>().OnHover();
                }
                else if (hit.collider.gameObject != mapHoverObject && hit.collider.gameObject.GetComponent<KS_MapMarker>())
                {
                    mapHoverObject.GetComponent<KS_Map_Marker>().OffHover();
                    mapHoverObject = hit.collider.gameObject;
                    mapHoverObject.GetComponent<KS_Map_Marker>().OnHover();
                }
            }
